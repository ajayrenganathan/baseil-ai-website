  ...
  return (
    <div className="px-7 py-5">
      ...
      <div className={`mb-5 transition-all duration-500 ${
        ...
        <div className="inline-flex items-center gap-2.5 px-3.5 py-2 rounded-lg bg-[#111916]/60 border border-[#52B788]/10">
          <Bot size={14} className="text-[#52B788]/60" />
          <span className="text-[0.78rem] font-[var(--font-outfit)] text-[#8FAF8A]">
            {CHAT_DATA.agent.name}
          </span>
          <span className="text-[0.68rem] font-[var(--font-outfit)] text-[#3D5A3A]">
      ...
      {showUserMessage && (
        <div className="flex gap-3 mb-5">
          <div className="w-7 h-7 rounded-full bg-[#52B788]/[0.08] flex items-center justify-center flex-shrink-0 mt-0.5">
            <User size={14} className="text-[#52B788]/50" />
          </div>
          <div className="rounded-lg bg-[#111916]/80 border border-[#52B788]/[0.06] px-4 py-2.5 max-w-[85%]">
            {isTypingUser ? (
              <TypingText
                text={CHAT_DATA.userMessage}
                charDelay={35}
                randomVariance={10}
                className="text-[0.9rem] font-[var(--font-outfit)] text-[#C8D8C4]"
              />
            ) : (
              <span className="text-[0.9rem] font-[var(--font-outfit)] text-[#C8D8C4]">
      ...
      {/* Tool call card */}
      {showToolCall && (
        <div className={`mb-5 ml-10 transition-all duration-500 ${
          showToolCall ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'
        }`}>
          <div className="rounded-lg border border-amber-500/10 border-l-2 border-l-amber-500/40 bg-[#111916]/60 px-4 py-3">
            <div className="flex items-center gap-2.5 mb-2">
              <Wrench size={13} className="text-amber-500/60" />
              <span className="text-[0.75rem] font-[var(--font-outfit)] text-amber-500/70">
                Tool call
              </span>
            ...
            {/* Progress bar */}
            <div className="mt-2.5 h-1 rounded-full bg-amber-500/10 overflow-hidden">
              <div
      ...
      {showBotResponse && (
        <div className="flex gap-3 mb-3">
          <div className="w-7 h-7 rounded-full bg-[#52B788]/[0.08] flex items-center justify-center flex-shrink-0 mt-0.5">
            <Bot size={14} className="text-[#52B788]/50" />
          </div>
          <div className="flex-1 min-w-0">
            {isTypingBot ? (
              <TypingText
                ...
                charDelay={18}
                randomVariance={6}
                className="text-[0.9rem] font-[var(--font-outfit)] text-[#C8D8C4] whitespace-pre-wrap"
              />
            ) : (
              <span className="text-[0.9rem] font-[var(--font-outfit)] text-[#C8D8C4] whitespace-pre-wrap">
      ...
      <div className={`flex items-center gap-4 mt-4 pt-3 border-t border-[#52B788]/[0.04] transition-all duration-500 ${
        ...
        <span className="flex items-center gap-1.5 text-[0.7rem] font-[var(--font-outfit)] text-[#3D5A3A]">
          <Clock size={11} /> {CHAT_DATA.metrics.duration}
        </span>
