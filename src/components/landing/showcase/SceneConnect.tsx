...
export function SceneConnect({ progress }: SceneConnectProps) {
  ...
  return (
    <div className="px-7 py-6">
      ...
      <div className="grid grid-cols-2 gap-3.5 max-w-[480px] mx-auto">
        {CONNECTIONS.map((conn, i) => {
          ...
          return (
            <div
              key={conn.name}
              className={`rounded-lg border bg-[#111916]/60 p-5 flex flex-col items-center gap-2.5 transition-all duration-700 ${
                visible
              ...
              style={{
                transitionDelay: isNew ? '0ms' : '0ms',
              }}
            >
              <div className={`w-11 h-11 rounded-lg flex items-center justify-center transition-colors duration-500 ${
                visible ? 'bg-[#52B788]/[0.08]' : 'bg-[#52B788]/[0.02]'
              }`}>
                <Database size={20} className="text-[#52B788]/60" />
              </div>
              <span className="text-[0.82rem] font-[var(--font-outfit)] text-[#C8D8C4] text-center">
                {conn.name}
              </span>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[0.68rem] font-[var(--font-outfit)] text-[#5A7A58]">
                  {conn.status}
                </span>
              </div>
            </div>
      ...
      {/* Counter */}
      <div className={`text-center mt-6 transition-all duration-700 ${
